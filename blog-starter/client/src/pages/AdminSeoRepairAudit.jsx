...
function previewFrom(log) {
  ...
  return { rows, fieldTotals, skippedRows, rollbackRisk, hiddenRows: Math.max(rows.length - 10, 0) };
}
...
export function AdminSeoRepairAudit() {
  ...
  function exportSelectedCsv() {
    if (!selectedLog || !preview.rows.length) { setError('CSV dışa aktarma için önce geri alma verisi olan bir log seç.'); return; }
  ...
  function exportSelectedJson() {
    if (!selectedLog) { setError('JSON dışa aktarma için önce bir log seç.'); return; }
  ...
  async function copySelectedSummary() {
    if (!selectedLog) { setError('Kopyalama için önce bir log seç.'); return; }
    const lines = [
